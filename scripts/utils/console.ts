...
const logErrors = (errors: ErrorLike[], gravity: 'error' | 'warning', verbose?: boolean): void => {
	errors.forEach((error, i) => {
		console.log(
			styleText(
				gravity === 'error' ? ['bgRed', 'white', 'bold'] : ['bgYellow', 'white', 'bold'],
				`${i + 1}/${errors.length}:`
			)
		);
		console.log(
			styleText([gravity === 'error' ? 'redBright' : 'yellowBright'], ` > ${error.message}`)
		);
		if (error.moduleName)
			console.log(
				styleText([gravity === 'error' ? 'redBright' : 'yellowBright'], 'Module: '),
				error.moduleName
			);
		if (error.file)
			console.log(
		...
		if (error.details)
			console.log(
				styleText([gravity === 'error' ? 'redBright' : 'yellowBright'], 'Details: '),
				error.details
			);
		if (error.stack && verbose)
			console.log(
				styleText([gravity === 'error' ? 'redBright' : 'yellowBright'], 'Stack: '),
				error.stack
			);
	});
};
